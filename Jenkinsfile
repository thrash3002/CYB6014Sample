...
def dotnet_build(){
	dir('Demo.NetCoreBuild.App') {
		sh(script: 'dotnet build Demo.NetCoreBuild.App.csproj', returnStdout: true);
	}
}

def dotnet_test(){
	dir('Demo.NetCoreBuild.Test') {
		
		sh(script: 'dotnet restore', returnStdout: true);
		sh(script: 'dotnet xunit -xml xunit-results.xml', returnStdout: true);
    }

    dir('Demo.NetCoreBuild.Transform') {
		sh(script: 'dotnet run ../Demo.NetCoreBuild.Test/xunit-results.xml xunitdotnet-2.0-to-junit-2.xsl junit-results.xml', returnStdout: true);
...
def dotnet_publish(){
    dir('Demo.NetCoreBuild.App') {
        sh(script: 'dotnet publish Demo.NetCoreBuild.App.csproj -o ./obj/Docker/publish', returnStdout: true);
